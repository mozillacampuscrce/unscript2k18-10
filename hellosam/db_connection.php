...
class DB_FUNCTIONS
{
    ...
    function view_excel1($t_id){
        $conn = mysqli_connect(DB_SERVER, DB_USER, DB_PASS, DB_NAME);
        $sql1 = "SELECT * FROM teachers WHERE t_email = '$t_id';";
        ...
        $sql="SELECT * FROM attend WHERE t_id='.$temp.';";
    ...
    public function check_attend($student_roll,$t_email){
        ...
        $sql = "SELECT * FROM attend WHERE s_roll = '$student_roll' and t_id='$temp' and attended='y'";
        $enter = mysqli_query($conn,$sql);
        $ret = @mysqli_num_rows($enter);
        if($ret>0){
            return false;
